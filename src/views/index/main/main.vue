  ...
  <div class="component">
    <el-card class="card">
      <div class="head" v-if="useRoute().path != '/following'">
        <el-menu v-model:default-active="mode" mode="horizontal" @select="change">
          <el-menu-item index="recommended" class="item">推荐</el-menu-item>
          <el-menu-item index="latest" class="item">最新</el-menu-item>
        </el-menu>
      </div>
      <ul class="list">
        <myli v-for="item in list" :key="item.id" :item="item" @click="router.push('/article_details/' + item.id)" />
      </ul>
    </el-card>
  </div>
</template>
